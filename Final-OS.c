...
//Create char array to hold command
char command[1000];
...
void LED(char * color, char * mode) {
    ...
    //Check if mode is blink
        ...
        if(strcmp(color, "red") == 0) {
            ...
            TA0CCR0 = 32787; // Fill to get 1 second @ 32 KHz
...
int main(void) {
    ...
    unsigned char byte; //Create variable to ready key press
    volatile unsigned int n; //Create delay variable

    Initialize_UART(); //Set it up
...
//This interrupt occurs after blink
...
__interrupt void LED_ISR() {
    ...
    //interrupt flag is automatically cleared for channel 0
}
...
char history[5][50]; //Create array of strings for history
...
void echo() {
    volatile unsigned int delay; //Create delay variable
    uart_write_string("Currently echoing press left button to stop:\n\r");

    //Loop until button is pressed
    while ((P1IN & BUT1) != 0) {

        unsigned char symbol; //Create variable to symbol

        symbol = uart_echo_char();
        //Slight delay loop
...
int main(void) {
    ...
    for(;;) {
        ...
        if(byte == '\r'){
            ...
            else if(strcmp(argument, "echo") == 0) echo();
